...
public static class CylinderGenerator
{
	...
	static PathVertex[] CalcNormals(Vector3[] localPoints)
	{
		...
		for (int i = 0; i < localPoints.Length; i++)
		{
			...
			if (i == 0)
			{
				tangent = (localPoints[i + 1] - localPoints[i]);
			}
			else if (i == localPoints.Length - 1)
			{
				tangent = (localPoints[i] - localPoints[i - 1]);
			}
			else
			{
				tangent = ((localPoints[i + 1] - localPoints[i]) + (localPoints[i] - localPoints[i - 1]));
			}
		...
		{
			Debug.Log(verts[i].tangent + " " + verts[i].normal+" "+ i);
		}*/
		return verts;
	}
