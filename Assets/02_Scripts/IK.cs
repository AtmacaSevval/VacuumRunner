...
public class IK
{
	...
	public void Solve(Vector3[] positions, Vector3 target)//target:fanusun üst kýsmý
	{
		...
		if (!hasCalculatedLengths)
		{
			...
			totalLength = 0;
			for (int i = 0; i < positions.Length - 1; i++)// length: iki nokta arasýndaki mesafe
			{
		...
		//Debug.Log(target + " " + positions[0] + " " + positions[positions.Length - 1]);
